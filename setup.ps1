...
if (-not $HOST_IP) {
    $HOST_IP = Read-Host "No se pudo detectar la IP. Ingrésala manualmente (ej. 192.168.1.42)"
}
...
Write-Host "v .env actualizado (HOST_IP=$HOST_IP)" -ForegroundColor Green
...
if (-not (Test-Path "athena\.env")) {
    ...
    Write-Host "v athena\.env configurado" -ForegroundColor Green
} else {
    Write-Host "v athena\.env ya existe — no se sobreescribio" -ForegroundColor Green
}
...
Write-Host "v apex_app\.env actualizado (EXPO_PUBLIC_API_URL=http://${HOST_IP}:8000)" -ForegroundColor Green
